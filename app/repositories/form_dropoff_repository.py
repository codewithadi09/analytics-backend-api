...
async def get_field_dropoff_stats() -> list[FieldDropoffRow]:
    async with get_connection() as conn:
        async with conn.cursor() as cur:
            await cur.execute(
                """
                WITH field_names AS (
                    SELECT field_name FROM rudder_schema.form_field_focus WHERE form_id = %(form_id)s
                    UNION
                    SELECT field_name FROM rudder_schema.form_field_complete WHERE form_id = %(form_id)s
                    UNION
                    SELECT field_name FROM rudder_schema.form_field_error WHERE form_id = %(form_id)s
                ),
                focus_counts AS (
